...
public class CommonServiceImpl implements CommonService {
	...
	@Override
	public PageInfo findPage(Class entityClass, int currentPage, int pageSize,
			Map<String, Object> accurateParams,
			Map<String, Object> vagueParams, String where, String order) {
		PageInfo pageInfo = new PageInfo();
		StringBuilder hql = new StringBuilder(); 
		hql.append("FROM " + entityClass.getSimpleName());
		List<Object> accurateValueList=new ArrayList<Object>();
		List<Object> vagueValueList=new ArrayList<Object>();
		boolean first=true;
		
		//精确条件参数
		if (accurateParams != null && !accurateParams.isEmpty()) {  
            Iterator<String> it = accurateParams.keySet().iterator();  
            String key = it.next();  
            if(first){
            	hql.append(" WHERE " + key + " = ?");
            	first=false;
            }else{
            	hql.append(" AND " + key + " = ?");
            }
            accurateValueList.add(accurateParams.get(key));
            while (it.hasNext()) {  
            	key = it.next();  
                hql.append(" AND " + key + " = ?");  
                accurateValueList.add(accurateParams.get(key));
		...
		//模糊条件参数
		if (vagueParams != null && !vagueParams.isEmpty()) {  
            Iterator<String> it = vagueParams.keySet().iterator();  
            String key = it.next();  
            if(first){
            ...
            vagueValueList.add(vagueParams.get(key));
            while (it.hasNext()) {  
            	key = it.next();  
                hql.append(" AND " + key + " like ?");  
                vagueValueList.add(vagueParams.get(key));
		...
		//条件参数 例如 datetime Between 2015/1/1 And 2016/1/1
		...
		int paramIndex = 0;
		//精确条件值
		for(int i=0;i< accurateValueList.size();i++){
			query.setParameter(paramIndex, accurateValueList.get(i));
			paramIndex++;
		}
		//模糊条件值
		for(int i=0;i< vagueValueList.size();i++){
			query.setParameter(paramIndex, "%"+vagueValueList.get(i)+"%");
			paramIndex++;
		}
		//记录范围
		if ((pageSize > 0) && (currentPage > 0)) {  
            query.setFirstResult((currentPage < 2) ? 0 : (currentPage - 1) * pageSize);  
            query.setMaxResults(pageSize);  
        }
		pageInfo.setDatas(query.list());
		
		try {
			//pageInfo.setRecordsTotal(getCount(null,entityClass));
			pageInfo.setRecordsFiltered(getCount(entityClass, accurateParams, vagueParams, where));
	...
	public long getCount(String where, Class entityClass) throws Exception {
		String hql = "SELECT COUNT(id) FROM " + entityClass.getSimpleName() + " WHERE ";
		if(where != null && where.length() > 3){
			hql += where;
		}else{
			hql += "1=1";
		}
		return Long.parseLong(sessionFactory.getCurrentSession().createQuery(hql).list().get(0).toString());
	}
	
	public long getCount(Class entityClass, Map<String, Object> accurateParams, Map<String, Object> vagueParams, String where)
			throws Exception {
		StringBuilder hql = new StringBuilder(); 
		hql.append("SELECT COUNT(id) FROM " + entityClass.getSimpleName());
		List<Object> accurateValueList=new ArrayList<Object>();
		List<Object> vagueValueList=new ArrayList<Object>();
		boolean first=true;
